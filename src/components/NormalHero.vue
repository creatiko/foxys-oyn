...
<template>
  <header
    id="home"
    class="relative flex min-h-svh w-full flex-col items-center justify-center overflow-hidden bg-surface-strong px-4 pb-20 pt-32.5 text-center sm:px-6"
  >
    <!-- Background video -->
    <video
    ...
    >
      <span
        ...
        :key="star.id"
        class="hero-star absolute rounded-full bg-inverse"
        :style="{
          left: star.left,
          top: star.top,
    ...
    <div class="relative z-10 mx-auto max-w-225">
      <h1
        class="font-display text-[clamp(2.5rem,7.2vw,3.9rem)] font-normal uppercase leading-[1.05] tracking-[0.01em] text-inverse text-shadow-(--hero-title-shadow)"
      >
        Come as you are

        <br />

        <em class="font-inherit italic text-accent">
          Stay till the music stops
        </em>
      </h1>

      <p
        class="mx-auto mt-4.5 max-w-180 font-body text-[clamp(1.22rem,2.1vw,1.6rem)] font-medium leading-[1.6] text-inverse"
      >
      ...
      <!-- Opening status -->
      <div
        class="mt-8.5 inline-flex items-center gap-2 rounded-full border border-inverse/20 [background:var(--hero-status-surface)] px-4.5 py-2.25 font-mono text-[0.96rem] tracking-wider text-inverse backdrop-blur"
        :class="{
    ...
    <button
      type="button"
      class="absolute bottom-5.5 left-1/2 z-10 -translate-x-1/2 font-mono text-[0.3rem] uppercase tracking-[0.16em] text-inverse/90"
    >
